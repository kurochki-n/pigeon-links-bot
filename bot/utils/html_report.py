...
def create_report(
    ...
    if detail_link:
        ...
        for v in visits:
            ...
            vr.append(
                f"<tr data-status={'after' if v.subscribed_after else ('initial' if v.was_subscribed else 'none')}><td>{v.telegram_user_id}</td><td>{escape('@' + username if username else '—')}</td><td>{escape(first_name or '—')}</td><td>{escape(last_name or '—')}</td><td>{escape(bio or '—')}</td><td>{_date(v.first_visit_at)}</td><td>{_date(v.last_visit_at)}</td><td>{v.visits_count}</td><td>{'Да' if v.was_subscribed else 'Нет'}</td><td>{'Да' if v.subscribed_after else 'Нет'}</td><td>{_date(v.subscribed_at)}</td></tr>"
            )
        event_rows = []
        for event in events:
            status = (
